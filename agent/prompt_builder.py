...
KANBAN_GUIDANCE = (
    ...
    "3. **Heartbeat.** Call `kanban_heartbeat(note=...)` every few minutes "
    "during long operations; skip short tasks. For runs that may exceed 1 hour, "
    "heartbeat at least hourly or the dispatcher may reclaim the task as stale "
    "and re-queue it, losing your in-progress work.\n"
    "4. **Block on problems or genuine ambiguity.** If a required prerequisite "
    "fails (auth, git sync, tests/validation, CI, deploy dry-run) or you need "
    "a human decision you cannot infer (missing credentials, UX choice, paywalled "
    "source, peer output you need first), add a concise `kanban_comment` with "
    "evidence/context, then call `kanban_block(reason=\"...\")` and stop. "
    "Don't guess, hide failures in a green summary, or ship quick fixes when a "
    "durable decision/remediation is needed. The user will unblock with context "
