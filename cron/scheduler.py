...
def _get_hermes_home() -> Path:
    ...
    Context-local profile overrides must win over the legacy module-level test
    hook. The hook is process-global, so using it for per-job profile switching
    can leak a profile job's home into unrelated parallel jobs.
    """
    try:
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    ...
    Some provider/config paths still load profile .env values through
