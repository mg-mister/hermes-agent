...
def _worker_skill_available(hermes_home: Optional[str], skill_name: str) -> bool:
    ...
    Dispatcher-spawned workers preload a small set of built-in worker skills
    with ``--skills``. When the worker activates a profile (``hermes -p
    <name>``), its ``SKILLS_DIR`` becomes ``<profile_home>/skills`` — which may
    not contain every skill that exists in the default root home. Preloading a
    missing skill is fatal at CLI startup (``ValueError: Unknown skill(s):
    ...``), aborting the worker before the agent loop runs. Gate each flag on
    actual resolvability.
    """
...
def _default_spawn(
    ...
    *,
    ...
    # the skills are deeper references. Users can configure profiles with
