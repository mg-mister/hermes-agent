...
def _worker_skill_available(hermes_home: Optional[str], skill_name: str) -> bool:
    ...
    worker activates a profile (``hermes -p <name>``), its ``SKILLS_DIR`` becomes
    ``<profile_home>/skills`` — which may not contain every skill that exists in
    the default root home. Preloading a missing skill is fatal at CLI startup
    (``ValueError: Unknown skill(s): ...``), aborting the worker before the agent
    loop runs. Gate each flag on actual resolvability.
    """
    from pathlib import Path as _Path

    # An unset HERMES_HOME means the worker falls back to the default root home
    # (``~/.hermes``), which ships bundled/profile skills.
...
def _default_spawn(
    ...
    *,
    ...
    # the skills are deeper references.
    #
    # Only add each flag when the skill actually resolves for the home the
    # worker runs under: profile skills are absent from some profile-scoped
    # skills dirs, and preloading a missing skill is fatal at CLI startup.
