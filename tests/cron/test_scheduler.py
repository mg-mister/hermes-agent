...
class TestParallelTick:
    ...
    def test_profile_job_does_not_leak_script_home_to_parallel_profileless_job(self, tmp_path, monkeypatch):
        """A profile job must not retarget a simultaneous profile-less job."""
        import threading
        ...
        monkeypatch.setattr(scheduler, "_hermes_home", mister_home)
        ...
        def coordinated_run_job(job):
            active_home = scheduler._get_hermes_home().resolve()
            if job["id"] == "profile-job":
                assert active_home == linearops_home.resolve()
                profile_context_entered.set()
                assert release_profile_job.wait(5), "profile-less job never ran"
            else:
                assert profile_context_entered.wait(5), "profile job never entered"
        ...
        jobs = [
            ...
            {"id": "profileless-job", "name": "profileless", "deliver": "local"},
        ]

        try:
            with patch("cron.scheduler.get_due_jobs", return_value=jobs), \
                 patch("cron.scheduler.advance_next_run"), \
                 patch("cron.scheduler.run_job", side_effect=coordinated_run_job), \
                 patch("cron.scheduler.save_job_output", return_value="/tmp/out.md"), \
                 patch("cron.scheduler._deliver_result", return_value=None), \
                 patch("cron.scheduler.mark_job_run"):
                from cron.scheduler import tick
            ...
            assert result == 2
            assert profile_context_entered.is_set()
            assert release_profile_job.is_set()
        finally:
            scheduler._shutdown_parallel_pool()
            scheduler._running_job_ids.clear()
