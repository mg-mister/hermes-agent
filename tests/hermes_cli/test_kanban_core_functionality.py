...
def test_default_spawn_auto_loads_builtin_worker_skills(kanban_home, monkeypatch):
    """Dispatcher argv includes built-in worker skills when they resolve.

    Kanban lifecycle guidance is injected via KANBAN_GUIDANCE, but Mister's
    local dispatcher policy also preloads the richer kanban-worker playbook and
    programming-agent routing skill when the assignee profile has them.
    """
    captured = {}
...
def test_default_spawn_appends_per_task_skills(kanban_home, monkeypatch):
    """Dispatcher argv carries built-in skills first, then per-task skills."""
    captured = {}
...
def test_default_spawn_dedupes_builtin_skills_from_task_skills(kanban_home, monkeypatch):
    """Explicit built-in task skills are not passed twice."""
    captured = {}
...
def test_dispatch_blocks_missing_forced_task_skills_before_spawn(
    ...
    that profile's HERMES_HOME. The dispatcher should catch that before spawn,
    block the task with actionable evidence, and avoid burning a worker slot.
    """
    monkeypatch.setattr(
        kb,
        "_worker_skill_available",
        lambda _home, skill: skill != "missing-specialist",
    )
    ...
    try:
        tid = kb.create_task(
            ...
            assignee="projectwikicurator",
            skills=["missing-specialist", "translation"],
        )
        ...
        assert "missing-specialist" in (run["summary"] or "")
    finally:
        conn.close()
